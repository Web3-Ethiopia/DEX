...
contract AllPoolManager {
    ...
    function createPool(
        ...
        ILiquidityPool.PoolPriceRange memory priceRange = ILiquidityPool
            .PoolPriceRange({
                minLowerBound: lowPrice,
                maxUpperBound: highPrice
            });

        address liquidityPoolAddress = ILiquidityPool(
            address(
                new LiquidityPool(
                    name,
                    token0,
        ...
        ).createPool(token0, token1, fee, priceRange);
    ...
    function removeLiquidity(
        ...
        (amount0, amount1) = ILiquidityPool(liquidityPoolMap[name])
            .removeLiquidity(liquidityPoolMap[name], liquidityAmount, _owner);
    ...
    function isMultiHopSwapPossible(
        ...
        (uint256 reserve1A, uint256 reserve1B) = ILiquidityPool(pool1)
            .getReserves(pool1);
        (uint256 reserve2A, uint256 reserve2B) = ILiquidityPool(pool2)
            .getReserves(pool2);
    ...
    function getProviderBasedOnPool(
        string memory name,
        address providerAddress
    ) public view returns (ILiquidityPool.Pool memory) {
        return
            ILiquidityPool(liquidityPoolMap[name]).getPoolDetails(
                liquidityPoolMap[name]
            );
    }

    function fetchLiquidityTokenReserves(
        string memory name
    ) public view returns (uint256 reserve0, uint256 reserve1) {
        return
            ILiquidityPool(liquidityPoolMap[name]).getReserves(
                liquidityPoolMap[name]
            );
    }

    function fetchLiquidityPoolAddress(
        string memory name
    ...
    function getMiniPool(
        ...
        return ILiquidityPool(liquidityPoolMap[poolKey]).getMiniPool(_owner);
    }
}
